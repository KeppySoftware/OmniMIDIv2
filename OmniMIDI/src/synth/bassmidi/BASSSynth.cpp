...
#if defined(_WIN32)
DWORD CALLBACK OmniMIDI::BASSSynth::AudioProcesser(void *buffer, DWORD length,
                                                   BASSSynth *me) {
    BASSInstance *bass = me->standard_instance;
    auto i = bass->ReadData((float *)buffer, length);
    return i;
}

DWORD CALLBACK OmniMIDI::BASSSynth::AudioEvProcesser(void *buffer, DWORD length,
                                                     BASSSynth *me) {
    BASSInstance *bass = me->standard_instance;
    BaseEvBuf_t *evbuf = me->ShortEvents;

    do
        bass->SendEvent(evbuf->Read());
...
DWORD CALLBACK OmniMIDI::BASSSynth::WasapiProc(void *buffer, DWORD length,
                                               void *user) {
    return AudioProcesser(buffer, length, (BASSSynth *)user);
}

DWORD CALLBACK OmniMIDI::BASSSynth::WasapiEvProc(void *buffer, DWORD length,
                                                 void *user) {
    return AudioEvProcesser(buffer, length, (BASSSynth *)user);
}

DWORD CALLBACK OmniMIDI::BASSSynth::AsioProc(int, DWORD, void *buffer,
                                             DWORD length, void *user) {
    return AudioProcesser(buffer, length, (BASSSynth *)user);
}

DWORD CALLBACK OmniMIDI::BASSSynth::AsioEvProc(int, DWORD, void *buffer,
                                               DWORD length, void *user) {
    return AudioEvProcesser(buffer, length, (BASSSynth *)user);
}
#endif
...
void OmniMIDI::BASSSynth::RenderingThread() {
    ...
    sleepRate = (int32_t)(((double)_bassConfig->BufPeriod /
                           (double)_bassConfig->SampleRate) *
                          10000000.0);
#endif
    ...
    case SingleThread:
        while (IsSynthInitialized()) {
            do
                standard_instance->SendEvent(ShortEvents->Read());
...
void OmniMIDI::BASSSynth::LoadSoundFonts() {
    ...
    case SingleThread:
    case Standard:
        if (!standard_instance->SetSoundFonts(SoundFonts)) {
...
bool OmniMIDI::BASSSynth::StartSynthModule() {
    ...
    case Standard: {
        ...
        uint32_t deviceFlags =
            (_bassConfig->MonoRendering ? BASS_DEVICE_MONO
                                        : BASS_DEVICE_STEREO);
        int device = _bassConfig->AudioEngine == Internal ? -1 : 0;
        if (!BASS_Init(device, _bassConfig->SampleRate, deviceFlags, NULL,
                       NULL)) {
            Error("Error Initializing BASS: %d", true, BASS_ErrorGetCode());
            return false;
        }
        Message("Initialized BASS.");

        try {
            standard_instance = new BASSInstance(ErrLog, _bassConfig, 16);
        } catch (std::exception &e) {
            Error("Failed to initialize BASSMIDI: %s", true, e.what());
            return false;
        }
        Message("Created BASSMIDI stream.");
...
#if defined(_WIN32)
        case WASAPI: {
            auto proc = _bassConfig->Threading == SingleThread
                            ? &BASSSynth::WasapiEvProc
                            : &BASSSynth::WasapiProc;

            if (!BASS_WASAPI_Init(
                    -1, _bassConfig->SampleRate,
                    _bassConfig->MonoRendering ? 1 : 2,
                    ((_bassConfig->Threading == Standard) ? BASS_WASAPI_ASYNC
                                                          : 0) |
                        BASS_WASAPI_EVENT,
                    _bassConfig->WASAPIBuf / 1000.0f, 0, proc, this)) {
                Error("Failed to initialize WASAPI: %d", true,
                      BASS_ErrorGetCode());
                BASS_Free();
                return false;
            }
            ...
            if (!BASS_WASAPI_Start()) {
                Error("Failed to start WASAPI: %d", true, BASS_ErrorGetCode());
                BASS_WASAPI_Free();
                BASS_Free();
                return false;
            }
        ...
        case ASIO: {
            ...
            if (asioCount < 1) {
                Error("No ASIO devices available", true);
                BASS_ASIO_Free();
                BASS_Free();
                return false;
            } else {
                Message("Detected %d ASIO devices.", asioCount);
            }

            if (!BASS_ASIO_Init(asioDev,
                                BASS_ASIO_THREAD | BASS_ASIO_JOINORDER)) {
                Error("Failed initialize ASIO: %d", true,
            ...
            if (BASS_ASIO_GetInfo(&asioInfo)) {
                Message("ASIO device: %s (CurBuf %d, Min %d, Max %d, Gran %d, "
                        "OutChans %d)",
                        asioInfo.name, asioInfo.bufpref, asioInfo.bufmin,
                        asioInfo.bufmax, asioInfo.bufgran, asioInfo.outputs);
            }

            if (!BASS_ASIO_SetRate(_bassConfig->SampleRate)) {
                Message("BASS_ASIO_SetRate failed, falling back to "
                        "BASS_ASIO_ChannelSetRate... BASSERR: %d",
                        BASS_ErrorGetCode());
                if (!BASS_ASIO_ChannelSetRate(0, 0, _bassConfig->SampleRate)) {
                    Message("BASS_ASIO_ChannelSetRate failed as well, BASSERR: "
                            "%d... This ASIO device does not want OmniMIDI to "
            ...
            for (uint32_t curSrcCh = 0; curSrcCh < asioInfo.outputs;
                 curSrcCh++) {
                ...
                if (strcmp(lCh, chInfo.name) == 0) {
                    leftChID = curSrcCh;
                    Message("%s >> This channel matches what the user "
                            "requested for the left channel! (ID: %d)",
                            chInfo.name, leftChID);
                }

                if (strcmp(rCh, chInfo.name) == 0) {
                    rightChID = curSrcCh;
                    Message("%s >> This channel matches what the user "
                            "requested for the right channel! (ID: %d)",
                            chInfo.name, rightChID);
                }
            ...
            if (leftChID == -1 && rightChID == -1) {
                leftChID = 0;
                rightChID = 1;
                Message("No ASIO output channels found, defaulting to CH%d for "
                        "left and CH%d for right.",
                        leftChID, rightChID);
            }
            ...
            if (noFreqChange) {
                ...
                if (devFreq == -1) {
                    Error("Failed to read the sample rate from the ASIO "
                          "device: %d",
                          true, BASS_ASIO_ErrorGetCode());
                    BASS_ASIO_Free();
                    BASS_Free();
                    return false;
                }
            ...
            if (_bassConfig->StreamDirectFeed)
                asioCheck = BASS_ASIO_ChannelEnableBASS(
                    0, leftChID, standard_instance->GetHandle(), 0);
            else
                asioCheck = BASS_ASIO_ChannelEnable(0, leftChID, proc, this);
            if (!asioCheck) {
                Error("Failed to enable ASIO stream: %d", true,
                      BASS_ASIO_ErrorGetCode());
                BASS_ASIO_Free();
                BASS_Free();
                return false;
            ...
            if (_bassConfig->StreamDirectFeed)
                Message("ASIO direct feed enabled.");
            else
                Message("AsioProc allocated.");

            Message("Channel %d set to %dHz and enabled.", leftChID,
                    _bassConfig->SampleRate);

            if (_bassConfig->MonoRendering)
                asioCheck =
                    BASS_ASIO_ChannelEnableMirror(rightChID, 0, leftChID);
            else
                asioCheck = BASS_ASIO_ChannelJoin(0, rightChID, leftChID);
            if (!asioCheck) {
                Error("Failed to set up ASIO audio channels: %d", true,
                      BASS_ASIO_ErrorGetCode());
                BASS_ASIO_Free();
                BASS_Free();
                return false;
            ...
            if (_bassConfig->MonoRendering)
                Message("Channel %d mirrored to %d for mono rendering. (F%d)",
                        leftChID, rightChID, _bassConfig->MonoRendering);
            else
                Message("Channel %d joined to %d for stereo rendering. (F%d)",
                        rightChID, leftChID, _bassConfig->MonoRendering);

            if (!BASS_ASIO_Start(0, 0)) {
                Error("Failed to start ASIO: %d", true,
                      BASS_ASIO_ErrorGetCode());
                BASS_ASIO_Free();
                BASS_Free();
                return false;
            ...
            BASS_ASIO_ChannelSetFormat(
                0, leftChID, BASS_ASIO_FORMAT_FLOAT | BASS_ASIO_FORMAT_DITHER);
            BASS_ASIO_ChannelSetRate(0, leftChID, _bassConfig->SampleRate);

            Message("asioDev %d >> chL %d, chR %d - asioFreq %d", asioDev,
                    leftChID, rightChID, _bassConfig->SampleRate);

            break;
        }
#endif
        default:
            Error("Unsupposed audio engine", true);
    ...
    case Multithreaded: {
        try {
            ...
            Error("BASSThreadManager intialization failed: %s", true, e.what());
            return false;
        }
        break;
    }
...
OmniMIDI::SynthResult OmniMIDI::BASSSynth::TalkToSynthDirectly(uint32_t evt,
    ...
    return standard_instance->SendDirectEvent(chan, evt, param)
               ? Ok
               : InvalidParameter;
}
