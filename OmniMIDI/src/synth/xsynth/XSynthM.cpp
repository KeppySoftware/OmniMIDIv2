...
bool OmniMIDI::XSynth::LoadSynthModule() {
    ...
    if (apiVer != libVer) {
        Error("Loaded incompatible XSynth version (%d.%d.X). Please use "
              "version %d.%d.X",
              true, libVer >> 8, libVer & 15, apiVer >> 8, apiVer & 15);
        UnloadSynthModule();
        return false;
    }
...
bool OmniMIDI::XSynth::StartSynthModule() {
    ...
    if (realtimeSynth.synth) {
        XSynth_Realtime_SendConfigEventAll(
            realtimeSynth, XSYNTH_CONFIG_SETLAYERS, _XSyConfig->LayerCount);
...
void OmniMIDI::XSynth::LoadSoundFonts() {
    ...
    if (_sfSystem.ClearList()) {
        ...
        for (size_t i = 0; i < _sfVecIter.size(); i++) {
            ...
            auto envOptions = XSynth_EnvelopeOptions{
                item.linattmod, item.lindecvol, item.lindecvol};
            ...
            if (!sfHandle.soundfont) {
                Error(
                    "An error has occurred while loading the SoundFont \"%s\".",
                    false, sfPath);
                continue;
            }
        ...
        if (SoundFonts.size() > 0) {
            XSynth_Realtime_SetSoundfonts(realtimeSynth, &SoundFonts[0],
                                          SoundFonts.size());
        }
    }
}
