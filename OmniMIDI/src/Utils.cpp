...
OMShared::Lib::Lib(const char *pName, const char *pSuffix,
                   ErrorSystem::Logger *PErr, LibImport **pFuncs,
                   size_t pFuncsCount) {
    ErrLog = PErr;
    Name = pName;
...
bool OMShared::Lib::IteratePath(char *outPath, OMShared::FIDs fid) {
    ...
    if (Utils.GetFolderPath(fid, buf, MAX_PATH_LONG)) {
        ...
        case OMShared::FIDs::UserFolder:
            printStatus =
                snprintf(outPath, MAX_PATH_LONG,
                         "%s/OmniMIDI/SupportLibraries/" LIBEXP(TYPE) "%s", buf,
                         Name, Suffix);
            break;

        case OMShared::FIDs::PluginFolder:
            printStatus = snprintf(outPath, MAX_PATH_LONG,
                                   "%s/OmniMIDI/Plugins/" LIBEXP(TYPE) "%s",
                                   buf, Name, Suffix);
            break;

        default:
            printStatus = snprintf(outPath, MAX_PATH_LONG,
                                   "%s/" LIBEXP(TYPE) "%s", buf, Name, Suffix);
            break;
        }
...
bool OMShared::Lib::GetLibPath(char *outPath) {
    ...
    int32_t st =
        snprintf(outPath, MAX_PATH_LONG, LIBEXP(TYPENO) "%s", Name, Suffix);
    if (st == -1)
        skip = true;
...
bool OMShared::Lib::LoadLib(char *CustomPath) {
    ...
    if (Library != nullptr) {
        ...
        if (Funcs != nullptr && FuncsCount != 0) {
            for (size_t i = 0; i < FuncsCount; i++) {
                if (Funcs[i].SetPtr(Library, Funcs[i].GetName()))
                    Message("%s --> 0x%08x", Funcs[i].GetName(),
                            Funcs[i].GetPtr());
            }
        }
        ...
        Error("The required library \"%s\" could not be found. This is "
              "required for the synthesizer to work.",
              true, Name);
...
bool OMShared::Lib::IsSupported(uint32_t loaded, uint32_t minimum) {
    ...
    if (libVer.GetHiWord() != libReqVer.GetHiWord()) {
        Error("OmniMIDI uses %s %d.%d bindings that are not compatible with "
              "the older %d.%d. Please update the library and try again.",
              true, Name, libReqVer.Build, libReqVer.Major, libVer.Build,
              libVer.Major);
        return false;
    }
    if (libVer.GetLoWord() < libReqVer.GetLoWord()) {
        Error(
            "OmniMIDI requires %s %d.%d.%d.%d minimum, but the loaded library "
            "reported %d.%d.%d.%d. Please update the library and try again.",
            true, Name, libReqVer.Build, libReqVer.Major, libReqVer.Minor,
            libReqVer.Rev, libVer.Build, libVer.Major, libVer.Minor,
            libVer.Rev);
        return false;
    }
...
OMShared::Funcs::Funcs() {
    ...
    auto v1 = (uint32_t(WINAPI *)(uint8_t, int64_t *))LibFuncs::GetFuncAddress(
    ...
    auto v2 = (uint32_t(WINAPI *)(int64_t *))LibFuncs::GetFuncAddress(
...
uint32_t OMShared::Funcs::QuerySystemTime(int64_t *v) {
    ...
    struct timeval tnow;
    gettimeofday(&tnow, 0);
    *v = tnow.tv_sec * (uint64_t)10000000 +
         (((369 * 365 + 89) * (uint64_t)86400) * 10000000);
    *v += tnow.tv_usec * 10;
    return 0;
#endif
}

bool OMShared::Funcs::GetFolderPath(const FIDs FolderID, char *path,
                                    size_t szPath) {
    if (path == nullptr)
        return false;
    ...
    return !(bool)SHGetFolderPathA(nullptr, csidl, NULL, SHGFP_TYPE_CURRENT,
                                   path);
#else
    ...
    case PluginFolder:
        ...
        if (envPath == nullptr || envPath[0] == 0) {
            ...
            static std::string configPath =
                std::format("{0}/.config", std::getenv("HOME"));
